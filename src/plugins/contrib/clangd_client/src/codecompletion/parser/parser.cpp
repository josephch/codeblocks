...
void Parser::OnLSP_CodeActionResponse(wxCommandEvent& event)
{
    ...
    for (auto& item : children)
    {
        ...
        std::cerr << "title : " << c.title << "command : " << c.command << " arguments : " << c.arguments << std::endl;
...
// ----------------------------------------------------------------------------
{
    std::cerr << "OnLSP_WorkspaceApplyEdit enter" << std::endl;
    if (GetIsShuttingDown())
        return;
    ...
    std::cerr << "workspace/applyEdit changes : " << changes << std::endl;
    for (auto& item : changes)
    {
        std::cerr << "workspace/applyEdit item : " << item << std::endl;
        wxString URI = item.first;
