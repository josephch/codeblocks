...
// ----------------------------------------------------------------------------
{
    ...
    if (pJson->contains("id"))
    {
        ...
        else if (idValue.StartsWith("textDocument/rangeFormatting"))
        {
            lspevt.SetString(idValue + STX + "result");
        }
...
// ----------------------------------------------------------------------------
{
    ...
    try { DidOpen(docuri, string_ref(strText.ToUTF8().data(), strText.Length()) ); }
    catch(std::exception &err)
    {
        //printf("read error -> %s\nread -> %s\n ", e.what(), read.c_str());
...
// ----------------------------------------------------------------------------
void ProcessLanguageClient::LSP_RequestRangeFormatting(cbEditor* pEd)
// ----------------------------------------------------------------------------
{
