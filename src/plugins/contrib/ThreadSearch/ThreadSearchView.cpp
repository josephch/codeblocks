...
BEGIN_EVENT_TABLE(ThreadSearchView, wxPanel)
    ...
    EVT_MENU(controlIDs.Get(ControlIDs::idOptionEncoded), ThreadSearchView::OnQuickOptions)
    ...
    EVT_UPDATE_UI(controlIDs.Get(ControlIDs::idOptionEncoded), ThreadSearchView::OnQuickOptionsUpdateUI)
...
void ThreadSearchView::OnBtnOptionsClick(wxCommandEvent &/*event*/)
{
    ...
    menu.AppendCheckItem(controlIDs.Get(ControlIDs::idOptionEncoded),
                         _("Encoded"), _("Consider files as not plain ascii"));
    menu.AppendSeparator();
...
void ThreadSearchView::OnQuickOptions(wxCommandEvent &event)
{
    ...
    else if (event.GetId() == controlIDs.Get(ControlIDs::idOptionEncoded))
    {
        findData.SetEncoded(event.IsChecked());
        hasChange = true;
    }
...
void ThreadSearchView::OnQuickOptionsUpdateUI(wxUpdateUIEvent &event)
{
    ...
    if (event.GetId() == controlIDs.Get(ControlIDs::idBtnSearch))
    {
        ...
        event.Check(findData.GetEncoded());
    else if (event.GetId() == controlIDs.Get(ControlIDs::idOptionResetAll))
    {
