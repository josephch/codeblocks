...
// ----------------------------------------------------------------------------
{
    ...
    LOGIT( "OnViewJumpTracker [%s] Checked[%d] IsShown[%d]",
            GetConfigBool("IsFloatingWindow")?"float":"dock",
...
// ----------------------------------------------------------------------------
{
    ...
    #if defined(LOGGING)
    LOGIT("cbEVT_DOCK_WINDOW_VISIBILITY[%d]" ,event.GetId() );
    #endif
    wxMenuBar* pbar = Manager::Get()->GetAppFrame()->GetMenuBar();
...
// ----------------------------------------------------------------------------
{
    ...
    EditorBase* pEdBase = event.GetEditor();
    #if defined(LOGGING)
        LOGIT( "JT OnEditorClosed Eb[%p][%s]", pEdBase, pEdBase->GetShortName().c_str() );
    #endif
...
// ----------------------------------------------------------------------------
{
    ...
    for (size_t count = 0; count < m_ArrayOfJumpData.GetCount(); ++count)
    {
        ...
        wxString msg = wxString::Format("[%zu][%s][%ld][%ld]", count, edFilename.c_str(), edPosn, edLine);
